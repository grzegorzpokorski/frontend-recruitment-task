...
const createTable = data => {
  ...
  const headingHTML = "<td>imie i nazwisko</td><td>email</td><td>adres</td><td>telefon</td><td>nazwa firmy</td>";
  heading.innerHTML = headingHTML;
  table.appendChild(heading);
  const createRow = item => {
    const tr = document.createElement("tr");
    const data = [item.name, item.email, item.address.city + ", " + item.address.street + ", " + item.address.suite, item.phone, item.company.name];
    data.forEach(i => {
